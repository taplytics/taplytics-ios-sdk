...
You can set an `accessibilityIdentifier` on your element so that the visual editor has an easier time to identify the element needed to apply the correct change. To enable this functionality, set `TaplyticsOptionMatchByAccessibilityIdentifier` to `@YES` in your starting options.
